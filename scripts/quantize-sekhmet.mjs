#!/usr/bin/env node
/**
 * Quantize scripts/pet-sources/sekhmet-draft.png → public/pets/sekhmet.png
...
const srcPath = join(root, "scripts/pet-sources/sekhmet-draft.png");
const outPath = join(root, "public/pets/sekhmet.png");
...
// Navy / black / gold / tan / red — Sekhmet desert palette
const PALETTE = [
  [0, 0, 0, 0],
  [18, 14, 12, 255], // outline near-black
  [28, 28, 36, 255], // dress black
  [42, 48, 72, 255], // navy shadow
  [58, 72, 112, 255], // navy mid
  [78, 96, 140, 255], // navy light
  [196, 148, 52, 255], // brass gold
  [232, 188, 72, 255], // bright gold
  [242, 210, 130, 255], // gold highlight
  [214, 176, 110, 255], // tan face
  [236, 208, 150, 255], // light tan
  [220, 64, 48, 255], // eye red
  [180, 48, 36, 255], // deep red / pad
  [236, 120, 64, 255], // orange pad
  [12, 12, 18, 255], // deep black
];
...
function nearestPalette(p) {
  ...
  // Bias reds so eyes + paw pads survive downsample
  const isRed = p[0] > 140 && p[0] > p[1] + 40 && p[0] > p[2] + 30;
  if (isRed) {
    const reds = PALETTE.filter(
      (c) => c[3] > 0 && c[0] > 150 && c[0] > c[1] + 30,
    );
    return reds.reduce((a, b) => (dist(p, a) < dist(p, b) ? a : b));
  }
  // Bias golds / yellows for headdress + paws
  const isGold =
    p[0] > 160 && p[1] > 120 && p[0] > p[2] + 40 && p[1] > p[2] + 20;
  if (isGold) {
    const golds = PALETTE.filter(
      (c) => c[3] > 0 && c[0] > 180 && c[1] > 130 && c[0] > c[2] + 30,
    );
    return golds.reduce((a, b) => (dist(p, a) < dist(p, b) ? a : b));
  }
...
console.log(`sekhmet ${tw}x${th} from draft`);
